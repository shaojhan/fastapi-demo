...
class TestUserRegistration:
    ...
    def test_user_registration_generates_unique_ids(self):
        ...
        測試每次註冊都會生成唯一的 ID
        """
        user1 = UserModel.register("user1", "password1", "user1@example.com")
        user2 = UserModel.register("user2", "password2", "user2@example.com")
    ...
    def test_user_registration_hashes_password(self):
        ...
        # 確保密碼已經被雜湊（在這個簡化的實現中，會加上 "hashed_" 前綴）
        assert user._hashed_password.password == f"hashed_{TEST_PASSWORD}"
        assert user._hashed_password.password != TEST_PASSWORD
...
class TestPasswordVerification:
    ...
    def test_password_verification_case_sensitive(self):
        ...
        測試密碼驗證是否區分大小寫
        """
        user = UserModel.register(TEST_UID, "MyPassword123", TEST_EMAIL)

        assert user.verify_password("MyPassword123") is True
        assert user.verify_password("mypassword123") is False
        assert user.verify_password("MYPASSWORD123") is False
    ...
    def test_password_verification_with_empty_password(self):
        ...
        測試使用空密碼進行驗證
        """
        user = UserModel.register(TEST_UID, "actual_password", TEST_EMAIL)
