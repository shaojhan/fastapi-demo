"""
Unit tests for KafkaRouter endpoints.
Tests HTTP layer for Kafka operations (Admin only).

測試策略:
- TestClient + dependency_overrides
- 驗證 Admin-only 授權
- 驗證各端點正確委派給 KafkaService
"""
import pytest
...
from app.router.KafkaRouter import router
...
class TestKafkaStatus:
    """測試 GET /kafka/status 端點"""

    def test_get_status_as_admin(self):
        from app.router.KafkaRouter import get_kafka_service
        app = _create_app()
        mock_service = MagicMock()
        mock_service.get_status.return_value = {"running": True, "subscriptions": []}

        app.dependency_overrides[get_current_user] = lambda: _make_admin()
        app.dependency_overrides[get_kafka_service] = lambda: mock_service
        client = TestClient(app)

        response = client.get("/kafka/status")
...
class TestKafkaProduce:
    """測試 POST /kafka/produce 端點"""

    def test_produce_message(self):
        from app.router.KafkaRouter import get_kafka_service
        app = _create_app()
        mock_service = MagicMock()
        mock_service.produce = AsyncMock()

        app.dependency_overrides[get_current_user] = lambda: _make_admin()
        app.dependency_overrides[get_kafka_service] = lambda: mock_service
        client = TestClient(app)

        response = client.post("/kafka/produce", json={
            "topic": "test-topic",
            "value": "test-value",
        })
        assert response.status_code == 200
...
class TestKafkaMessages:
    ...
    def test_get_messages(self):
        ...
        response = client.get("/kafka/messages?page=1&size=10")
        assert response.status_code == 200
